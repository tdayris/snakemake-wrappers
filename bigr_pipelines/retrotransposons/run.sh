...
declare -x PIPELINE_PATH="${PIPELINE_PREFIX}/bigr_pipelines/retrotransposons"
...
if [ "${SUMMARY}" != "" ]; then
  ...
  snakemake -s "${SNAKEFILE_PATH}" --configfile "config.yaml" --profile "${SNAKEMAKE_PROFILE_PATH}/${PROFILE}" "${SNAKE_ARGS[@]}" && message INFO "retrotransposons pipeline successful" || error_handling "${LINENO}" 2 "Error while running retrotransposons quant pipeline"
fi
