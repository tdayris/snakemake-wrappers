...
if [ ! -f "v.2.1.0.GRCh38.cluster.config.json" ]; then
  rsync --verbose --checksum --human-readable --progress --partial "${PIPELINE_PATH}/v.2.1.0.GRCh38.cluster.config.json" "v.2.1.0.GRCh38.cluster.config.json"
fi

if [ ! -f ".config.gleaves" ]; then
  touch ".config.gleaves"
fi
