...
if [ "${SUMMARY}" != "" ]; then
  message CMD "snakemake -s ${SNAKEFILE_PATH} --configfile config.yaml --cache salmon_index --profile ${SNAKEMAKE_PROFILE_PATH} ${SNAKE_ARGS[*]} --summary > ${SUMMARY}"
  ...
  message CMD "snakemake -s ${SNAKEFILE_PATH} --configfile config.yaml --cache salmon_index --profile ${SNAKEMAKE_PROFILE_PATH} ${SNAKE_ARGS[*]} --rulegraph | dot -Tpng > ${GRAPH}"
  ...
  message CMD "snakemake -s ${SNAKEFILE_PATH} --configfile config.yaml --cache salmon_index --profile ${SNAKEMAKE_PROFILE_PATH} ${SNAKE_ARGS[*]}"
