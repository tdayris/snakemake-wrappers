...
snakemake -s "${PIPELINE_PATH}" --profile "${SNAKEMAKE_PROFILE_PATH}" --cache eacon_install post_process_eacon_databases eacon_databases && message INFO "EaCoN successful" || error_handling "${LINENO}" 2 "Error while running Oncoscan pipeline"
