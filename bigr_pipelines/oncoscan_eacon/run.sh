...
declare -x PIPELINE_PATH="${PIPELINE_PREFIX}/meta/bio/eacon_oncoscan"
...
snakemake -s "${PIPELINE_PATH}/test/Snakefile" --cache eacon_install eacon_databases --profile "${SNAKEMAKE_PROFILE_PATH}" && message INFO "EaCoN successful" || error_handling "${LINENO}" 2 "Error while running Oncoscan pipeline"
