rule bowtie2_map:
    ...
    threads: 20
    resources:
        mem_mb=lambda wildcards, input, attempt: get_8gb_per_attempt(wilcards, input.sample, attempt),
        time_min=lambda wildcards, input, attempt: get_4h_per_attempt(wilcards, input.sample, attempt),
        tmpdir="tmp",
    log:
