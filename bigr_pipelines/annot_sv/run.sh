...
declare -x PIPELINE_PATH="${PIPELINE_PREFIX}/bigr_pipelines/annot_sv"
...
if [ "${SUMMARY}" != "" ]; then
  ...
  snakemake -s "${SNAKEFILE_PATH}" --configfile "config.yaml" --profile "${SNAKEMAKE_PROFILE_PATH}/${PROFILE}" "${SNAKE_ARGS[@]}" && message INFO "Annotation successful" || error_handling "${LINENO}" 2 "Error while running AnntSV pipeline"
fi
