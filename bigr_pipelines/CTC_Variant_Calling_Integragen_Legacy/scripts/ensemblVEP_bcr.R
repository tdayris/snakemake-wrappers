...
for (invcf in files_to_process) {
    ...
    if(length(as.character(seqnames(input_brc)))>0){
        name_normal <- NA
        name_tumor <- gsub(
            pattern = "",
            replacement = "",
            x = basename(path = invcf)
        )    

        CHROM <- as.character(x = seqnames(input_brc))
        POS <- as.character(x = start(input_brc))
        REF <- as.character(x = granges(input_brc)$REF)
        ALT <- NULL    

        for (i in 1:length(rowRanges(input_brc)$ALT)) {
            for (j in 1:length(rowRanges(input_brc)$ALT[[i]])) {
                if (is.null(ALT[i]) || is.na(ALT[i])) {
                    ALT[i] <- as.character(
                        x = rowRanges(input_brc)$ALT[[i]][[j]]
                    )
                } else {
                    if (as.character(rowRanges(input_brc)$ALT[[i]][[j]]) == "") {
        ...
        rownames(input_brc) <- paste(
            CHROM, POS, REF, ALT,
            rownames(x = input_brc),
            sep = "_"
        )
        csq <- parseCSQToGRanges(
            input_brc,
            VCFRowID = rownames(x = input_brc)
        )
        csq_df <- as.data.frame(csq, row.names = NULL)
        csq_df_aggregated <- aggregate(
            csq_df,
            list(csq_df$VCFRowID),
            paste,
            collapse = ","
        )
        ...
        Patient <- rep(x = Patient, length(x = CHROM))
        SampleName <- name_tumor
        SampleName <- rep(x = SampleName, length(x = CHROM))
        Method <- rep(x = "bamreadcount", length(x = CHROM))
        normal_ref_count <- rep(x = NA, length(x = CHROM))
        normal_alt_count <- rep(x = NA, length(x = CHROM))
        normal_depth <- rep(x = NA, length(x = CHROM))
        normal_alt_freq <- rep(x = NA, length(x = CHROM))
        identifiers <- names(rowRanges(input_brc))
        identifiers_splitted <- strsplit(
            x = identifiers, split = "|", fixed = TRUE
        )
        tumor_ref_count <- unlist(lapply(identifiers_splitted, "[[", 1))
        for (i in 1:length(tumor_ref_count)) {
          tumor_ref_count[i]<-unlist(strsplit(tumor_ref_count[i], split="_", fixed=TRUE))[5]
        }
        tumor_ref_count_splitted <- unlist(strsplit(tumor_ref_count, split="_", fixed=TRUE))[5]
        tumor_alt_count <- unlist(lapply(identifiers_splitted, "[[", 2))
        tumor_depth <- unlist(lapply(identifiers_splitted, "[[", 3))
        tumor_alt_freq <- unlist(lapply(identifiers_splitted, "[[", 4))
        varID <- unique(names(csq))
        STRAND <- csq_df_aggregated$strand
        FILTER <- as.character(granges(input_brc)$FILTER)
        ...
        Feature_RefSeq_splitted <- strsplit(x = Feature_RefSeq, split = ",")
        Canonical_NM <- NA
        ...
        if(!identical(pos_in_canonical_vectors[[1]], integer(0)) && !is.integer(pos_in_canonical_vectors)){
          for (i in 1:length(pos_in_canonical_vectors)) {
            ...
            }else if(is.integer(pos_in_canonical_vectors)) {
              Canonical_NM[i] <- pos_in_canonical_vectors[1]
            }else{
              Canonical_NM[i] <- NA
            }
          }    
          
          Protein_position_splitted <- strsplit(x = Protein_position, split = ",")
        ...
        EXON_splitted <- strsplit(csq_df_aggregated$EXON, ",")
        inExon <- NA
        ...
        table_to_write <- data.frame(
            ...
            CODING, inExon, BIOTYPE, Position_Type, SIFT, PolyPhen, dbSNP_COSMIC,
            uniqueID
        )
        write.table(
            table_to_write,
        ...
        rm(
            ...
            Canonical_NM,Canonical_Protein_position,CHROM,CODING,
            Consequence,Consequence_splitted,csq,csq_df,csq_df_aggregated,
            dbSNP_COSMIC,EXON_splitted,Feature_RefSeq,Feature_RefSeq_splitted,
            FILTER,Gene_Name,Hotspot,i,IMPACT,IMPACT_splitted,inExon,j,Method,
            name_normal,name_tumor,normal_alt_count,normal_alt_freq,normal_depth,
            normal_ref_count,input_brc,Patient,PolyPhen,POS,
            ...
            tmp,tumor_alt_count,tumor_alt_freq,tumor_depth,tumor_ref_count,
