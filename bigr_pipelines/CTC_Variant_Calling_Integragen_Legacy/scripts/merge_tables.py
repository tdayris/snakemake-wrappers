...
df = read_big_table(path="bigtable.uniq.tsv")
labels = read_labels(path="EGFR_Targeted NGS_Sample sheet_21-07-2022-1.csv")

print(df.shape)
print(df.head())
print(df["KEY"].sort_values().head())
print(labels.shape)
print(labels["KEY"].sort_values().head())
...
print(merged.shape)
print(merged.head())
merged.to_csv("BigTableAnnoteted.tsv", sep="\t", header=True, index=False)