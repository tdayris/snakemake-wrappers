...
for (invcf in files_to_process) {
    ...
    name_normal <- NA
    name_tumor <- base::rownames(colData(input_hc))
    CHROM <- base::as.character(x = seqnames(input_hc))
    POS <- base::as.character(x = start(input_hc))
    REF <- base::as.character(x = granges(input_hc)$REF)
    ALT <- NULL
    ...
    csq <- parseCSQToGRanges(input_hc, VCFRowID = base::rownames(x = input_hc))
    csq_df <- base::as.data.frame(csq, row.names = NULL)
    ...
    for (i in base::strsplit(x = csq_df_aggregated$SYMBOL, split = ",")) {
        tmp <- c(tmp, list(i %in% hotspot_list$Hotspot_SYMBOL))
    }

    Hotspot <- sapply(tmp, paste, collapse = ",")
    Patient <- base::strsplit(x = name_tumor, split = "_")[[1]][1]
    Patient <- base::rep(Patient, base::length(x = CHROM))
    SampleName <- name_tumor
    SampleName <- base::rep(SampleName, base::length(x = CHROM))
    Method <- base::rep("haplotypecaller", base::length(x = CHROM))
    normal_ref_count <- base::rep(NA, base::length(x = CHROM))
    normal_alt_count <- base::rep(NA, base::length(x = CHROM))
    normal_depth <- base::rep(NA, base::length(x = CHROM))
    normal_alt_freq <- base::rep(NA, base::length(x = CHROM))
    tumor_ref_count <- NULL

    for (i in 1:length(geno(input_hc)$AD[, name_tumor])) {
        tumor_is_null <- base::is.null(tumor_ref_count[i])
        tumor_is_na <- base::is.na(tumor_ref_count[i])
        if (tumor_is_null || tumor_is_na) {
            tumor_ref_count[i] <- base::as.character(
                x = geno(input_hc)$AD[, name_tumor][[i]][[1]]
            )
        } else {
            predicate <- base::as.character(
                x = geno(input_hc)$AD[, name_tumor][[i]][[1]]
            )
            if (predicate == "") {
                tumor_ref_count[i] <- base::paste(
    ...
    for (i in 1:length(geno(input_hc)$AD[, name_tumor])) {
        if (is.null(tumor_alt_count[i]) || is.na(tumor_alt_count[i])) {
            tumor_alt_count[i] <- base::as.character(
    ...
    tumor_depth <- base::rowSums(
        cbind(
            base::as.numeric(x = tumor_ref_count), 
            base::as.numeric(x = tumor_alt_count)
        )
    )

    tumor_alt_freq <- base::as.numeric(x = tumor_alt_count) / tumor_depth
    ...
    pos_in_canonical_vectors <- sapply(
        base::strsplit(x = csq_df_aggregated$CANONICAL, split = ","),
        function(y) which(y == "YES")
    )
    Feature_RefSeq_splitted <- base::strsplit(
    ...
    Protein_position_splitted <- base::strsplit(
    ...
    Amino_acids_splitted <- base::strsplit(Amino_acids, split = ",")
    Canonical_Amino_acids <- NULL

    for (i in 1:length(pos_in_canonical_vectors)){
        if (length(pos_in_canonical_vectors[[i]]) > 0) {
            for (j in 1:length(pos_in_canonical_vectors[[i]])){
                if (is.null(Canonical_Amino_acids[i])) {
                    Canonical_Amino_acids[i] <- Amino_acids_splitted[[i]][pos_in_canonical_vectors[[i]][j]]
                } else {
                    Canonical_Amino_acids[i] <- base::paste(
    ...
    Consequence_splitted <- base::strsplit(x = Consequence, split = ",")
    Canonical_Consequence <- NULL

    for (i in 1:length(pos_in_canonical_vectors)) {
        if (length(pos_in_canonical_vectors[[i]]) > 0) {
            for (j in 1:length(pos_in_canonical_vectors[[i]])){
                if (is.null(Canonical_Consequence[i])) {
                    Canonical_Consequence[i] <- Consequence_splitted[[i]][pos_in_canonical_vectors[[i]][j]]
                } else {
                    Canonical_Consequence[i] <- base::paste(
    ...
    CODING <- base::grepl("HIGH|MODERATE", IMPACT)
    EXON_splitted <- base::strsplit(x = csq_df_aggregated$EXON, split = ",")
    test <- sapply(EXON_splitted, function(y) which(y != "NA"))
    inExon <- base::lengths(x = test)
    ...
    uniqueID <- base::paste(SampleName, varID, sep = "_")
    table_to_write <- base::data.frame(
