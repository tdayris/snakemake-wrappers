...
rule sed_rename_sample_wbc_brc:
    input:
        "vep/brc/{sample}_{version}_{manip}.orig.tsv"
    output:
        temp("vep/brc/{sample}_{version}_{manip}.renamed.tsv")
    threads: 1
    resources:
        mem_mb=512,
        time_min=10,
        tmpdir=tmp,
    log:
        "logs/vep/rename/brc/{sample}_{version}_{manip}.wbc.log"
    conda:
        "../envs/bash.yaml"
    params:
        replace=(
...
rule sed_rename_sample_baseline_brc:
    input:
        "vep/brc/{sample}.baseline.orig.tsv"
    output:
        temp("vep/brc/{sample}.baseline.renamed.tsv")
    threads: 1
    resources:
        mem_mb=512,
        time_min=10,
        tmpdir=tmp,
    log:
        "logs/vep/rename/brc/{sample}.baseline.log"
    params:
        replace=(
