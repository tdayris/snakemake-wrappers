...
rule grep_pass:
    input:
        "gatk/{mutect_dir}/{sample}.vcf"
    output:
        temp("gatk/{mutect_dir}/{sample}.filtered.vcf"),
    threads: 1
    resources:
        mem_mb=get_1gb_per_attempt,
        tmpdir=tmp,
...
rule ensembl_vep_mutect:
    input:
        ...
        fasta="resources/GRCh38.fasta",
    output:
        vcf=temp("vep/annotate/{sample}.mutect2_wbc.vcf"),
    threads: 1
    resources:
        mem_mb=get_20gb_per_attempt,
        time_min=get_3h_per_attempt,
        tmpdir=tmp,
    log:
        "logs/vep/{sample}.mutect2_wbc.log",
    params:
        "--species homo_sapiens "
        "--assembly GRCh38 "
        "--cache "
...
rule ensemblvep_mutect:
    ...
    output:
        tsv=temp("vep/mutect2_wbc/{sample}.tsv"),
    threads: 1
    resources:
        mem_mb=get_10gb_per_attempt,
        time_min=get_45min_per_attempt,
        tmpdir=tmp,
    log:
        "logs/vep/mutect2_wbc/{sample}.log",
    params:
        organism=config.get("vep_db", "hg38"),
    container:
        str(
