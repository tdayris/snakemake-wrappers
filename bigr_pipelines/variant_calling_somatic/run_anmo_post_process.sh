...
SNAKEFILE_PATH="${PIPELINE_PATH}/post_process_ANMO.smk"
SNAKE_ARGS=()
PROFILE="slurm"
SUMMARY=""
GRAPH=""
...
if [ "${SUMMARY}" != "" ]; then
  ...
  message CMD "snakemake -s ${SNAKEFILE_PATH} --profile ${SNAKEMAKE_PROFILE_PATH}/${PROFILE} ${SNAKE_ARGS[*]} --runegraph | dot -Tpng > ${GRAPH}"
  ...
  snakemake -s "${SNAKEFILE_PATH}" --profile "${SNAKEMAKE_PROFILE_PATH}/${PROFILE}" "${SNAKE_ARGS[@]}" && message INFO "Variant calling successful" || error_handling "${LINENO}" 2 "Error while running variant calling pipeline"
fi
