    ...
    for col in complete_design.columns:
        ...
        for l1, l2 in itertools.combinations(levels, 2):
            ...
            for level, ref in [[l1, l2], [l2, l1]]:
               ...
               # two by two.
               try:
                  if any(i in contains for i in [f"test_{level}", f"reference_{ref}", f"test_{l1}_vs_reference_{l2}"]):
...
def yield_samples(complete_design: pandas.DataFrame,
    ...
    for col in complete_design.columns:
        ...
        for l1, l2 in itertools.combinations(levels, 2):
            ...
            # two by two.
            try:
               if any(i in contains for i in [f"test_{l1}", f"reference_{l2}", f"test_{l1}_vs_reference_{l2}", f"test_{l2}_vs_reference_{l1}"]):
